// Archivo: script.js (Contiene toda la lógica JS)

// =============================================================
// ✅ FUNCIÓN ASÍNCRONA: ASISTENTE DE BÚSQUEDA W3C (PROXY DEMO)
// Llama al servidor de Netlify para simular una búsqueda externa o dar una respuesta clave.
// =============================================================
async function fetchW3cSearch(query) {
    ...
    // 
    // Opción A: Simulación de respuesta relevante (Si la función de Netlify solo devuelve un mensaje)
    // 
    // Simula el tiempo de red y procesamiento del servidor (2 segundos)
    await new Promise(resolve => setTimeout(resolve, 2000)); 

    // SIMULACIÓN DE DEVOLUCIÓN DEL RESULTADO RELEVANTE (Se dirige al contenido extraído)
    const resultHtml = `
    ...
    /*
    // 
    // Opción B: Si quieres que la búsqueda REDIRECCIONE directamente a W3C (más simple)
    // 
    const encodedTerm = encodeURIComponent(query);
    const w3cSearchUrl = `https://www.w3.org/search/?q=${encodedTerm}`;
    window.location.href = w3cSearchUrl;
    */
}

// 
// =============================================================
// ✅ FUNCIÓN DE BÚSQUEDA DEDICADA (FINAL)
// Se invoca al presionar Enter o hacer clic en Buscar.
// =============================================================
function searchContent() {
    ...
    if (query.length === 0) {
        renderSection(location.hash || '#home');
        return;
    }

    // 1. Identificar si la búsqueda debe ir al servicio externo (W3C/Netlify Proxy)
    const w3cKeywords = ['w3c', 'accesibilidad', 'wcag', 'wai', 'estandares web', 'html', 'css', 'javascript', 'iso', 'idioma', 'etiqueta', 'bcp 47', 'cors', 'proxy', 'servidor', 'backend'];
    ...
    // 2. Búsqueda interna normal (en las secciones de tu guía)
    ...
    if (!matchFound) {
        // Si no encuentra nada internamente, ofrece la búsqueda externa W3C como alternativa.
        resultsHtml += `
            <div class="w3c-card error">
                <h3>No se encontraron resultados en esta guía.</h3>
                <p>¿Quizás quieres buscar **"${query}"** en el **asistente de W3C**?</p>
                <button onclick="fetchW3cSearch('${query}')" class="w3c-btn">
                    Buscar en Asistente W3C
                </button>
            </div>
        `;
    }
...
// 
// =============================================================
// ✅ FUNCIÓN PARA CARGAR DATOS (NETLIFY FUNCTION DEMO)
// Se mantiene si tienes la sección #estandares-w3c
// =============================================================
async function fetchW3cStandards() {
    ...
    try {
        // Asumiendo que esta es la Netlify Function que creaste para el proxy real
        ...
        container.innerHTML = `
            <div class="w3c-card success">
                ...
                <p><strong>URL de origen:</strong> <code>${data.source || 'Desconocida'}</code></p>
                <p><strong>Título Extraído:</strong> ${data.title || 'Información cargada con éxito.'}</p>
...
// 
// =============================================================
// ✅ FUNCIONES DE UTILIDAD Y EJERCICIOS (Se mantienen intactas)
// =============================================================
...
// Clase Desarrollador necesaria para el ejercicio de POO
class Desarrollador {
...
// 
// =============================================================
// ✅ LÓGICA DE NAVEGACIÓN (Crucial)
// =============================================================
// Función para renderizar el contenido de una sección
function renderSection(hash) {
    const contentElement = document.getElementById('content');
    const hashKey = hash.substring(1) || 'home'; // Elimina el # y usa 'home' por defecto

    if (sections[hashKey]) {
        contentElement.innerHTML = sections[hashKey];
        ...
        contentElement.innerHTML = `
            <section id="error">
                ...
                <p>La sección solicitada (${hash}) no existe.</p>
                <a href="#home" onclick="renderSection('#home')">Volver al inicio</a>
            </section>
        `;
    }
}
...
// 
// =============================================================
// ✅ Contenido dinámico por sección (sections)
// =============================================================
const sections = {
